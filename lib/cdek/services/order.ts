...
// UI-хелперы блока «Создать отправление» вынесены в order-ui.ts (клиентобезопасно,
// без графа postgres.js) и реэкспортятся здесь для серверных потребителей/тестов.
export {
  shipmentCreateUiState,
  type ShipmentCreateUiState,
} from './order-ui';
