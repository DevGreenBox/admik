...
// ---------------------------------------------------------------------------
// Дефолтное хранилище токена: Redis при REDIS_URL, иначе память.
//
// ВНИМАНИЕ, НЕ ПУТАТЬ С RATE-LIMIT. Раньше здесь стояла отсылка «как
// getDefaultLimiter», и она вводила в заблуждение: у лимитера деградация в
// память — это ОБХОД ЗАЩИТЫ, поэтому там стоит fail-closed-гвард (в production
// без REDIS_URL он бросает, см. resolveRateBackend в lib/auth/rate-limit.ts).
// Здесь деградация безобидна: кеш короткоживущего OAuth-токена СДЭК per-instance
// означает лишь лишний запрос токена, а не дыру. Гвард тут НАМЕРЕННО не нужен —
// падать без Redis значило бы ломать доставку на ровном месте.
//
// Отсылка убрана осознанно: сегодня проект дважды обжёгся на комментариях,
// объявлявших две функции эквивалентными там, где в коде они разошлись
// (isTbankMock vs TbankManager.isMock; getDefaultLimiter vs getStorefrontLimiter).
// ---------------------------------------------------------------------------
