 ...
 *
 * SECURITY (доработка находки #11). Здесь жила ВТОРАЯ, независимая реализация той
 * же проверки — регулярка `v.startsWith('/') || /^https?:\/\/\S+$/i…`. Она была
 * слабее CMS-версии и пропускала protocol-relative «//evil.com», а через неё идёт
 * ссылка ГЕРОЯ главной (в неё обёрнут весь первый экран витрины) и пункты меню
 * шапки. Ровно тот паттерн «две функции с одним именем и разными правилами».
 * Теперь правило одно на весь репозиторий — lib/security/safe-href.ts.
 */
const hrefSchema = z.string().trim().min(1).refine(isSafeHref, SAFE_HREF_MESSAGE);
