...
2. **rsync кода Admik:**
   `rsync -az --delete --exclude '.env' --exclude .git --exclude node_modules --exclude .next --exclude 'THE CASE' --exclude backups -e "ssh -i $ADMIK_DEPLOY_SSH_KEY" ./ "$ADMIK_DEPLOY_HOST":/opt/admik/`
