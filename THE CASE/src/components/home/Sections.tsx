...
/**
 * Блок фото-отзывов сообщества «ВЫ + THE CASE» на главной. Контент и показ — из
 * настроек Admik (settings.home.reviews). По умолчанию блок СКРЫТ (enabled:false):
 * магазин включает его в админке (opt-in, как ValuesStrip). Если выключен —
 * секция не рендерится (ранний null). Если задан photos — реальные фото; иначе
 * плейсхолдеры «Ваше фото» (как на странице /reviews). Тексты «THE CASE» приходят
 * из настроек/фолбэка (HOME_FALLBACK.reviews), а не захардкожены в компоненте.
 */
export function CommunityReviews({
  reviews = HOME_FALLBACK.reviews,
}: {
  reviews?: ResolvedHome["reviews"];
}) {
  ...
  return (
    <section className="section-space-sm">
      <div className="container-brand">
        ...
        <FadeIn>
          <Link href={reviews.ctaHref} className="link-editorial mt-12 inline-block">
            {reviews.ctaLabel}
          </Link>
        </FadeIn>
      </div>
