...
/** Плитка категории на главной: ссылка + описание из админки вместо статов. */
export interface CategoryTile extends CategoryLink {
  description: string;
}

/**
 * Категории ВЕРХНЕГО уровня плитками для главной: до `max`, порядок — `sort` из
 * админки (приходит из репозитория уже отсортированным).
 *
 * Отличие от categoryLinks: тот делает DFS (родители + дети вперемешку) — для
 * футерного списка это уместно, для плиток нет: ребёнок встал бы рядом с
 * родителем. Здесь — только топ-уровень.
 *
 * `description` — поле категории из админки: владелец пишет туда подпись плитки
 * («4 длины», «Клинический крой»). Пустое описание — нормально, плитка просто
 * без подписи. Пустое дерево → пустой массив: вызывающий скрывает секцию.
 */
export function topCategoryLinks(
  categories: AdmikCategoryDto[],
  max = 3,
): CategoryTile[] {
  return categories.slice(0, Math.max(0, max)).map((c) => ({
    slug: c.slug,
    name: c.name,
