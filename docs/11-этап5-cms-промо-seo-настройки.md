...
- **CMS** — отключаемый модуль (`cms`, флаг и права `cms.read`/`cms.write` уже заведены). Витрина
  THE CASE может работать как headless-каталог без CMS; Gang Auto — с CMS-страницами и акциями.
...
- `size_charts`: `{ charts:[{ id, title, note?, genders:string[], columns:[{key,label}], rows:Record<string,string>[] }], footnote? }` — размерные сетки магазина (см. 5.4.8). Платформенный дефолт — `{ charts: [] }`
...
- **`0035_shop_settings_size_charts_seed.sql`** (2026-07-20) — по тому же образцу сидирует ключ
  `size_charts` значением `'{}'::jsonb`. Пустой объект → при чтении (`mergeSettings`) раздел падает
  на платформенный дефолт `{ charts: [] }`, и блок размерной сетки на витрине просто не рисуется.
  Администратор получает готовую строку для правки через `/admin/settings/size-charts` без ручного
  `INSERT`. **В миграции нет и не может быть значений конкретного магазина** (набор сеток, колонок
  и цифр у каждого ИМ свой) — контент выносится в `db/seed/*` (см. 5.4.8).
...
- **`sizeCharts`** — размерные сетки отдаются витрине целиком (структура таблицы + сноска), они
  публичны по определению: их видит покупатель на карточке товара (5.4.8).
...
### 5.4.8. Размерные сетки (ключ `size_charts`) — реализовано 2026-07-20

> **Что это.** Таблица «Размерная сетка» на карточке товара витрины. До 2026-07-20 она была
> **константой `SIZE_TABLE` в компоненте витрины** с выдуманными цифрами: данные одного магазина
> лежали в коде платформы, а покупатель видел мерки, которых никто не измерял. Теперь это
> **настройка магазина** — тот же механизм, что `home`/`navigation`.

**Контракт значения** (`sizeChartsSchema`, `lib/settings/schemas.ts`):

```jsonc
{
  "charts": [
    {
      "id": "women",                                  // slug, уникален в массиве (refine)
      "title": "Женские размеры",
      "note": "рост 165–172",                         // опц. подпись под заголовком
      "genders": ["women"],                           // [] = «сетка применима всегда»
      "columns": [                                    // ПРОИЗВОЛЬНЫЕ колонки, 1..24
        { "key": "size",  "label": "Размер" },
        { "key": "chest", "label": "Грудь, см" }
      ],
      "rows": [ { "size": "44 S", "chest": "88" } ]    // до 200 строк; ключи = key колонок
    }
  ],
  "footnote": "Замеры по изделию, допуск ±1 см"        // опц. общая сноска
}
```

**Мультитенантность — ключевое.** В коде **не зашита ни одна сетка и ни одна колонка**. Набор
колонок задаёт контент-менеджер, поэтому один и тот же компонент обслуживает и медформу, и обувь,
и любой будущий магазин. Платформенный дефолт — **пустой** `{ charts: [] }`: пока магазин не завёл
сетки, блок (и кнопка «Размерная сетка») на витрине **не рендерится вовсе**. Данные конкретного
магазина живут в `db/seed/*`, а не в миграции: `db/seed/the-case-size-charts.sql` (сетки THE CASE,
проверяется `tests/seed/the-case-size-charts.test.ts`).

**Ограничения схемы** (кривой/раздутый JSONB не должен ломать рендер витрины): ≤24 сеток, 1..24
колонки, ≤200 строк, длины строк ограничены; `.strip()` — анти-tamper, как у прочих ключей.
Дополнительно `superRefine` ловит две ошибки контент-менеджера: **дубль `key` колонки** (две
колонки схлопнулись бы в одну ячейку и дали одинаковый React-key) и **ячейку без колонки** (она не
рендерится, но занимает место в JSONB, в ответе витрины и в `audit_log`). Дубли `id` отвергает
`.refine` — форма их НЕ «чинит» молча, владелец видит ошибку вместо потери одной из сеток.

**Правило выбора сетки по полу** (`selectSizeCharts`, чистая функция витрины
`THE CASE/src/lib/size-table.ts`, тест `size-table.test.ts`):

1. сеток нет → `[]` (блок не рисуется);
2. сетка с пустым `genders` подходит всегда;
3. иначе — если нормализованный пол товара (`trim` + `lowercase` + `ё→е`) есть в `genders`;
4. **не подошла ни одна** (unisex, пусто, мусор) → показываются **ВСЕ** сетки, вкладками.

Пункт 4 — осознанное исправление прежнего поведения `gender === "men" ? men : women`, при котором
unisex и неизвестный пол **молча** получали женскую таблицу. Когда пол неизвестен, честнее
показать все сетки, чем угадать одну.

> ### ⚠️ Правило для контент-менеджера: `genders` должен содержать `women` или `men`
>
> Адаптер витрины **сужает** пол товара до трёх значений — `women` / `men` / `unisex`
> (`resolveGender`, `THE CASE/src/lib/admik/adapter.ts`: распознаёт `women|female|жен`,
> `men|male|муж`, `unisex|унисекс`, иначе — вывод из slug категорий, иначе `unisex`). До
> `selectSizeCharts` доходит **только** одно из этих трёх значений.
>
> Поэтому сетка с `genders: ["женский"]` или `genders: ["Ж"]` **не совпадёт никогда** — и товар
> вместо своей сетки получит ВСЕ сетки вкладками (правило 4). Каждая сетка обязана содержать
> `women` либо `men` (или пустой `genders`, если применима всем).
>
> Второй источник той же поломки — **обнулённый `attributes_cache`** (пол товара исчез): см.
> «Мина в данных» в `docs/05 §4.5`.

**Server Action.** `updateSizeChartsAction` (`lib/settings/action-factory.ts`),
`permission: 'settings.manage'`. Значение сохраняется **целиком (replace, не merge)** — порядок
сеток/колонок/строк задаёт админ. Обязательно зовёт `invalidateCache()` (иначе memo эффективных
настроек отдаёт старое значение до рестарта: «сохранил — витрина не видит») и ревалидирует
`/admin/settings` + витринные пути. Аудит — `settings.size_charts.update` (before/after).

**Admin UI** — `/admin/settings/size-charts` (см. 5.4.5). Сборка значения из формы — чистая
`buildSizeChartsPayload` (`lib/settings/size-charts-form.ts`): тримит всё, отбрасывает пустые
сетки/колонки/строки, не кладёт пустые `note`/`footnote` в объект. Строки хранятся в состоянии
формы **по индексу колонки** (`string[][]`), а не по ключу, — чтобы переименование `key` колонки
не теряло уже введённые данные.

**Витрина.** Публичное поле DTO — `sizeCharts` (`lib/storefront/settings-dto.ts`,
`GET /api/storefront/v1/settings`); DTO копирует **значения**, а не ссылки на объекты настроек,
чтобы его нельзя было изменить «сквозь» кэш эффективных настроек. Рендер — `SizeGuide.tsx`
(колонки из `chart.columns`, ячейки через `getCell`: читаются только own-свойства строки, иначе
ключ вида `toString`/`constructor` вернул бы функцию из `Object.prototype`).

**Накат данных магазина.** `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f db/seed/the-case-size-charts.sql`.
⚠️ После **прямого SQL** нужен **рестарт `app`-контейнера**: `invalidateSettingsCache()` вызывается
только из Server Action, внешний SQL memo не сбрасывает (ADR-019, `docs/02`).

**Тесты:** `tests/settings/size-charts-schema.test.ts`, `tests/settings/size-charts-form.test.ts`,
`tests/seed/the-case-size-charts.test.ts`, `tests/storefront/settings-dto.test.ts`,
`tests/storefront/settings-route.test.ts`, `THE CASE/src/lib/size-table.test.ts`.
...
### 5.1.8. Безопасность ссылок: единый `lib/security/safe-href.ts` (закрыта находка #11 аудита)

**Проблема.** Проверка ссылки существовала в **трёх** независимых копиях с **разными** правилами:
`hrefSchema` в `lib/cms/schemas.ts`, одноимённая `hrefSchema` в `lib/settings/schemas.ts` и
проверка на рендере витрины. Копия в настройках была слабее (регулярка вместо разбора URL) и
пропускала protocol-relative `//evil.com` — а через неё идёт ссылка **героя главной страницы**, в
которую обёрнут весь первый экран витрины.

**Решение.** Один модуль `lib/security/safe-href.ts` (`isSafeHref` / `safeHref` /
`SAFE_HREF_MESSAGE`), к которому сведены обе Zod-схемы. Правило меняется в одном месте — меняется
везде.
...
| Паттерн | Риск |
|---|---|
| `javascript:` / `data:` / `vbscript:` (регистронезависимо, с ведущими управляющими символами) | хранимый XSS при клике |
| `java\nscript:` («размазанная» схема) | WHATWG URL-парсер выбрасывает TAB/LF/CR — поэтому схема сверяется по **схлопнутой** строке |
| `//host` (protocol-relative) | open-redirect: браузер подставит текущую схему |
| **любой обратный слэш `\`** | для special-схем WHATWG приравнивает `\` к `/`: `new URL('/\evil.com','https://shop.ru').href === 'https://evil.com/'` |

Последняя строка — тот самый обход, из-за которого находка была закрыта лишь **частично**: условие
`v.startsWith('/') && !v.startsWith('//')` для `/\evil.com` истинно, и Next рендерил
`<a href="/\evil.com">` дословно, а браузер уводил покупателя на чужой домен.

**Разрешено:** корневой относительный путь `/…`, якорь `#…`, абсолютный URL со схемой
`http`/`https`/`mailto`/`tel`. Потолок длины — 2048.

**Два рубежа:** `isSafeHref` — на входе (Zod-схемы CMS и настроек), `safeHref` — на рендере (для
строк, уже лежащих в БД: записанных до появления валидации или напрямую через SQL).

> ⚠️ **Зеркало на витрине.** У витрины (`THE CASE/`) отдельный `tsconfig`, импорт из корня admik
> невозможен — там лежит копия модуля `THE CASE/src/lib/safe-href.ts` со своим тестом. Любое
> изменение правил здесь **обязано** быть повторено там; оба набора тестов гоняют один список
> payload'ов, а страж-тест `tests/security/safe-href-mirror.test.ts` не даёт копиям разойтись.

**Тесты:** `tests/security/safe-href.test.ts`, `tests/security/safe-href-mirror.test.ts`,
`THE CASE/src/lib/safe-href.test.ts`, `tests/cms/schemas.test.ts`.
...
## 6. Влияние на референсные витрины

| Витрина | CMS (5.1) | Промо N×M (5.2) | SEO (5.3) | Настройки (5.4) |
|---|---|---|---|---|
| **THE CASE** (без брендов) | модуль `cms` может быть **выключен** — работает как headless-каталог без CMS-страниц | акции опциональны | sitemap без раздела брендов (пустая `brands`), products/categories — есть | брендинг THE CASE через `shop_settings` |
| **Gang Auto** (с акциями) | CMS-страницы (промо-лендинги, о компании) включены | «3 по 2» / scope-скидки на категории/бренды — ключевой сценарий | sitemap с брендами, акции-бейджи | свои `module_overrides` (catalog+orders+cms), порог бесплатной доставки |
