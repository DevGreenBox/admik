...
| `care` | `care` | `уход` | `""` |
| `color` | `color` | `цвет` | `""` (уровень товара; вариантная ось цвета — §3.6) |
...
- **Уточнение 2026-07-20.** Вариантный уровень характеристик теперь наполняется (матрица
  «цвет × размер» пишет цвет в `product_attributes` варианта, `docs/05 §4.4`), и цвет доезжает до
  витрины отдельными полями DTO (§3.6). Но метка **размера** по-прежнему берётся из **имени**
  варианта — и это не временный компромисс, а инвариант: из имён строится ось `sizes[]` и фасет
  размеров каталога, поэтому цвет в имя не подмешивается.
...
### 3.6. Цвет как вариантная ось (реализовано 2026-07-20)

Раньше цвет был **одной строкой на весь товар** (`attributes_cache.color` уровня товара) и годился
только как подпись. С матрицей «цвет × размер» в админке (`docs/05 §4.4`) цвет стал свойством
**варианта**, и контракт расширен аддитивно:

| Поле DTO | Уровень | Смысл |
|---|---|---|
| `variant.color` | вариант | метка цвета из справочника («Белый»); `null` — у варианта цвета нет |
| `variant.colorHex` | вариант | `#RRGGBB` из `attribute_values.color_hex`; `null` — HEX не задан |
| `product.colors[]` | товар | ось цветов `{ value, hex }` для селектора; собирается `collectProductColors` из **активных** вариантов |

- **`colors[]` строится из вариантных значений**, а не из `attributes_cache` товара: одна строка
  на весь товар матрицу не описывает. Только активные варианты — неактивный цвет выбрать некуда.
- **HEX нормализуется на границе DTO** (`normalizeColorHex`): наружу отдаётся только валидный
  `#RRGGBB`, иначе `null`. Витрина подставляет значение в inline-стиль свотча, поэтому мусор из БД
  до стиля не доходит. Если HEX не задан — рисуется текстовая метка.
- **Палитра — данные, не код.** HEX задаётся в справочнике значений (`docs/05 §2.4`), витрина не
  содержит таблицы «название цвета → HEX»: иначе новый магазин с «Пыльной розой» требовал бы правки
  кода платформы.
- ⚠️ **Имя варианта остаётся меткой РАЗМЕРА.** Цвет в `variant.name` не подмешивается: из имён
  строится ось `sizes[]` товара и фасет размеров каталога. Адаптер поэтому уникализирует размеры
  (`[...new Set(variants.map(v => v.size))]`) — при матрице метки размеров повторяются у вариантов
  разных цветов.
- **Списочный DTO** цвет варианта не несёт (вариантов в списке нет): адаптер собирает
  `colors: [{ value: dto.color, hex: null }]` из цвета уровня товара — для фасета этого достаточно.

### 3.7. Размерные сетки (`sizeCharts`)

Таблица размеров на карточке приходит **из настроек магазина** — публичное поле DTO `sizeCharts`
(`GET /api/storefront/v1/settings`), полный контракт и правила — `docs/11 §5.4.8`. На витрине:

- выбор подходящих сеток — чистая функция `selectSizeCharts(charts, gender)`
  (`THE CASE/src/lib/size-table.ts`, юнит-тест `size-table.test.ts`), рендер — `SizeGuide.tsx`;
- колонки рендерятся из `chart.columns` — **зашитых сеток и колонок в коде витрины нет**;
- сеток нет → блок и кнопка «Размерная сетка» не рендерятся вовсе;
- пол не совпал ни с одной сеткой (unisex/пусто/мусор) → показываются **ВСЕ** сетки вкладками.

> ⚠️ **Связка с §3.2:** пол берётся из `resolveGender`, который сужает значение до
> `women`/`men`/`unisex`. Сетка с `genders: ["женский"]` не совпадёт никогда — правило для
> контент-менеджера см. `docs/11 §5.4.8` и `docs/14 §10`.
...
interface CartItem {
  ...
  quantity: number;
  color?: string;           // метка цвета выбранного варианта (2026-07-20) — СТРОГО опциональна
}
```

> ⚠️ **Почему `color` опционален (и почему это правило для любого нового поля позиции).** Корзина
> живёт в **localStorage** (`zustand persist`): у покупателя может лежать корзина, сохранённая
> предыдущей версией витрины. Обязательное поле сломало бы такие корзины при первом же чтении.
> `undefined` = цвет не выбирался (у товара нет цветовой оси) либо позиция из старой корзины.
> Ровно так же в своё время добавляли `available`. Ключ для сервера — по-прежнему только
> `variantId`: цвет здесь — снимок для UI, на расчёт он не влияет (anti-tamper, ADR-010).
